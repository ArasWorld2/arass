...
function startWebServer(client) {
    ...
    app.post('/api/shoutout', async (req, res) => {
        try {
            ...
            if (!targetChannelId) {
                console.error('[Roblox API Error] SO_CHANNEL_ID variable is missing in environment variables.');
            ...
            if (!channel) {
                try {
                    ...
                    return res.status(200).json({ 
                        success: false, 
                        error: `Bot cannot access channel ${targetChannelId}. Check bot permissions or channel ID.` 
                    });
                }
            }
